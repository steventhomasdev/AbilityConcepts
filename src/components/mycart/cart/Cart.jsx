...
export default function Cart({ isLogin, setCartCount }) {
  ...
  const OnRemoveButtonClick = (event) => {
    ...
    RemoveItemsFromCart(userData)
    .then((data) => setCartCount(data.body.quantity))
    setTimeout(() => {  setCartItemRefresh(true); }, 500);
  }
  ...
  const getCartTotal = () => {
    ...
    for (let i in cartItems) {
      totalPrice += Number(getItemTotal(cartItems[i].productDetails.productprice, cartItems[i].quantity).replace(/\,/g,''));
    }
                ...
                {cartItems?.map((product) => (
                  <tr>
                    ...
                    <td class="cart_description">
                      <h4>
                        <a onClick={onProductClick} id={[product._id]}>{product.productDetails.productName}</a>
                      </h4>
                    ...
                    <td class="cart_total">
                      <p class="cart_total_price">${ getItemTotal(product.productDetails.productprice,product.quantity) }</p>
                    </td>
                    <td class="cart_delete">
                      <a class="cart_quantity_delete" onClick={OnRemoveButtonClick} id={[product.productId]}>
              ...
              <div class="col-sm-6">
                <div class="total_area">
                  <ul>
                    ...
                    <li>
                      Total <span>${Number(getCartTotal().replace(/\,/g,'')) + calculateTax()}</span>
                    </li>
                  </ul>
